...
class BloodSugarCard extends HTMLElement {
  ...
  setConfig(config) {
    if (!config || !config.entity) {
      // Avoid hard failure on cast clients that may not pass full config.
      this._config = {
        name: "Blood Sugar",
        history_minutes: 30,
        trend_window: 3,
      ...
      this._configError = "Missing required 'entity' in card config.";
      this._render();
      return;
    }
    ...
    if (this._configError) {
      this.shadowRoot.innerHTML = `
        <style>
          ...
          .card {
            background: var(--bs-unknown, #424242);
            color: #fff;
            border-radius: 18px;
            padding: 18px 20px;
        ...
        <div class="card">
          ...
          <div class="message">Configuration missing required fields.</div>
          <div class="detail">${this._configError}</div>
        </div>
      `;
      return;
    }
